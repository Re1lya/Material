...
| 停止态全自动 | In progress | 受限 Stopped auto-merge Task 已发布到生产 Pipeline（generation 11）并在 PR #11 上完成全自动合并：校验、自动合并、commit status 与合并后 main PipelineRun 均成功，生成分支自动删除；Argo 仍为 OutOfSync 且 automated sync 未开启 | 开启 scoped auto-sync 并完成 Stopped 零 NPU 全自动验收 | 推理平台 |
